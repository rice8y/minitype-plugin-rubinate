import { callWasmSync } from "./wasm-sync.js";
...
export function tokenizeLinderaSync(text: string, options: TokenizerOptions): Morpheme[] {
  const raw = callWasmSync(options.dictionary, "analyze", {
  ...
  const matches = options.correspondence && raw.length ? callWasmSync("jmdict_furigana", "lookup", {
    kana: "hiragana",
